package com.nju.rjxy.refactorbackend.codeTopics.postprocess;
        ...
        for(int topic = 0; topic <model.K; topic++) {
            ...
            for (Map.Entry<String, List<String>> service : serviceFiles.entrySet()) {
                ...
                for (String file : files) {
                    ...
                    int fileWords = model.z[fileID].size();
                    serviceProbability += (double) fileWords / serviceWordCount * fileProb; //公式中的p(t|ms)
                }
...
//    public List<FunctionalTopic> findMergeServiceFunctionalTopic(double tc_threshold, double file_threshold, double service_threshold) {
//        List<FunctionalTopic> ret = new ArrayList<>();
//        // 主题与服务的相关度 - 和主题词汇无关
//        List<Map<String,Double>> topicServiceProb = getTopicServiceProb();
//        for (int topic = 0; topic < model.K; topic++) {
